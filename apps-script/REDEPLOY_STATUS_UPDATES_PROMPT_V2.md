# Redeploy Prompt v2 — Apps Script Status Updates (self-contained for fresh Codex)
...
The Rowell Family Reunion 2026 website (`https://rowell-family-reunion.netlify.app/`) just shipped a new admin feature called "Status Updates" — a CRUD tool that lets the operator publish announcements to the group, with an option to broadcast each update by email to every registered member.

The frontend (admin page, public page, sub-nav, main nav) is already live on Netlify. The **backend Apps Script that powers it has NOT yet been redeployed**, so the new POST handlers `status_update_create` and `status_update_delete` are not in the live `/exec` URL yet. Your job is to redeploy the existing Apps Script project as a NEW VERSION of the existing deployment — preserving the live `/exec` URL — and run a no-broadcast smoke test to confirm the round-trip works.
...
- **Latest `Code.gs` source (always fetch fresh):** `https://raw.githubusercontent.com/ZoSlay/rowell-reunion-site/main/apps-script/Code.gs` — currently 659 lines, ends with `function jsonResponse(obj) { ... }`
...
2. **First substitution:**
   ...
   - Click **Replace all** — must report exactly **8 occurrences replaced**. If the count differs, STOP and report the actual number.
...
4. Confirm the admin sub-nav shows three tabs: `Dashboard | Documents | Status Updates` (Status Updates active).
5. Fill the publish form:
   ...
   - **"Email this update to all registered members" checkbox: UNCHECK IT** (this is the critical guardrail — do not fire a real broadcast).
6. Click **Publish Update**.
7. Confirm the green success banner reads: `Published as #N. Not emailed (checkbox was unchecked).` where N is a small integer (the row number in the Status Updates sheet tab).
...
```
Step 2 final line count after paste: <should be 659>
Step 2 trailing-junk check: <clean / had to delete N extra lines>
Step 3 Sheet ID substitution count: <should be exactly 8>
...
Step 7 publish update_id: <N>
Step 7 admin list shows test row: <yes / no>
Step 7 public updates.html shows test row: <yes / no>
Step 7 delete removes from both views: <yes / no>
...
Stop after Step 8. Do not perform any additional actions. Do not send a real broadcast email.
